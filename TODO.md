...
| Experimentalists Anonymous | Boss OC-2, Moog Taurus, Univox Microphaser, Tau 1010, Nobels CO-2 and PH-D, and about 20 more by one to three rows | Rows that came from a wide pairing pass on scans, applied during the OCR-pass work and rolled back before its commit; the export built in between still had them. Real values among them (OC-2 R48 470K, Taurus R76 7.5k, Microphaser C1-C4 50n, Tau C5/C6 18pF) are gone until the pairing can take a wider pass without the junk that pass also brought |
| Experimentalists Anonymous | Obesifier, and scans in general | Junk pairs survive: R12H = 11, bare one-digit resistor values ('R38 = 8'), mangled designators |
